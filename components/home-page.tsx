...
export function HomePage() {
  ...
  const [respondingTo, setRespondingTo] = useState<CalculationNode | null>(
    null
  );
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header
        className="border-b bg-card sticky top-0 z-50 backdrop-blur supports-backdrop-filter:bg-card/80"
        role="banner"
      >
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
      ...
      >
        <ErrorBoundary>
          <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
              ...
              <div className="space-y-6">
                ...
                <Card className="animate-in fade-in-0 duration-300">
                  ...
                  <CardContent>
                    {error && (
                      <div
                        className="text-sm text-destructive mb-4 p-3 bg-destructive/10 rounded-md"
                        role="alert"
                      >
                        {error}
                      </div>
                    ...
                    {isLoadingCalcs ? (
                      ...
                      <div className="text-center py-12 space-y-3">
                        ...
                        <p className="text-muted-foreground">
                          No discussions yet.{" "}
                          {user ? "Start one!" : "Login to start one!"}
                        </p>
                      </div>
                    ) : (
                      <CalculationTree
            ...
            <aside
              className="space-y-6"
              aria-label="User actions and information"
            >
              {user ? (
              ...
              <Card className="animate-in fade-in-0 slide-in-from-bottom-4 duration-500 delay-150">
                ...
                <CardContent className="text-sm text-muted-foreground space-y-2">
                  <p>
                    Welcome to Number Discussions - a social network where
                    people communicate through math!
                  </p>
      ...
      <footer className="border-t bg-card" role="contentinfo">
        <div className="container mx-auto px-4 py-4 text-center text-sm text-muted-foreground">
          <p>
            &copy; {new Date().getFullYear()} Number Discussions. All rights
            reserved.
          </p>
        </div>
      </footer>
